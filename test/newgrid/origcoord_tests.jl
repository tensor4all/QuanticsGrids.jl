@testitem "origcoord functions - basic 1D grid" begin
    ...
    @test grididx_to_origcoord(grid, (2^4,)) ≈ 1.0 - 1.0 / 2^4
    ...
    @test quantics_to_origcoord(grid, quantics) ≈ expected_origcoord
    @test origcoord_to_quantics(grid, expected_origcoord) == quantics
end
...
@testitem "origcoord functions - 2D grid" begin
    ...
    @test all(isapprox.(grididx_to_origcoord(grid, (2^3, 2^4)), (2.0 - 2.0 / 2^3, 3.0 - 2.0 / 2^4)))

    # Test middle values
    mid_grididx = (2^2 + 1, 2^3 + 1)  # Middle of each dimension
    expected_origcoord = (0.0 + 2.0 * 2^2 / 2^3, 1.0 + 2.0 * 2^3 / 2^4)
    @test all(isapprox.(grididx_to_origcoord(grid, mid_grididx), expected_origcoord))
...
@testitem "origcoord functions - different base" begin
    ...
    @test all(isapprox.(grididx_to_origcoord(grid, (base^2, base^3)), (1.0 - 2.0 / base^2, 6.0 - 6.0 / base^3)))
    ...
    for _ in 1:20
        ...
        @test all(isapprox.(quantics_to_origcoord(grid, quantics), origcoord))
        @test origcoord_to_quantics(grid, origcoord) == quantics
    end
end
...
@testitem "origcoord functions - stress test" begin
    ...
    for _ in 1:30
        ...
        grididx2 = quantics_to_grididx(grid, quantics)
        origcoord2 = grididx_to_origcoord(grid, grididx2)

        @test all(isapprox.(quantics_to_origcoord(grid, quantics), origcoord2))
        @test origcoord_to_quantics(grid, origcoord2) == quantics
    end
end
...
@testitem "edge cases - maximum R values and large numbers" begin
    ...
    @test quantics_to_grididx(grid, max_quantics) == 2^large_R
    @test grididx_to_quantics(grid, (1,)) == min_quantics
    @test grididx_to_quantics(grid, (2^large_R,)) == max_quantics
...
@testitem "edge cases - degenerate grids with R=0" begin
    ...
    @test quantics_to_grididx(grid, Int[]) == (1, 1, 1)

    # Test origcoord functions
    @test grididx_to_origcoord(grid, (1, 1, 1)) == (0.0, 0.0, 0.0)
...
@testitem "edge cases - mixed R values including zeros" begin
    ...
    for _ in 1:20
        ...
        grididx_local = quantics_to_grididx(grid, valid_quantics)
...
@testitem "edge cases - extreme base values" begin
    ...
    for _ in 1:20
        ...
        @test grididx_to_quantics(grid2, grididx) == quantics
    end
end
...
@testitem "edge cases - extreme coordinate ranges" begin
    ...
    for _ in 1:10
        ...
        origcoord = grididx_to_origcoord(grid2, grididx)

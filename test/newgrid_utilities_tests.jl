@testitem "localdimensions" begin
    ...
    grid = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,))
    @test QuanticsGrids.localdimensions(grid) == fill(2, R)

    grid_2d = QuanticsGrids.NewDiscretizedGrid((R, R); lower_bound=(0.0, 0.0), upper_bound=(1.0, 1.0))
    @test QuanticsGrids.localdimensions(grid_2d) == fill(2, 2R)

    # Test with different base values
    grid_base3 = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), base=3)
    @test QuanticsGrids.localdimensions(grid_base3) == fill(3, R)

    grid_base4 = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), base=4)
    ...
    indextable = [
        [QuanticsGrids.QuanticsIndex(:x, 1), QuanticsGrids.QuanticsIndex(:y, 1)],  # site 1: 2 indices
        [QuanticsGrids.QuanticsIndex(:x, 2)],                                      # site 2: 1 index  
        [QuanticsGrids.QuanticsIndex(:y, 2)],                                      # site 3: 1 index
        [QuanticsGrids.QuanticsIndex(:x, 3), QuanticsGrids.QuanticsIndex(:y, 3), QuanticsGrids.QuanticsIndex(:x, 4)]  # site 4: 3 indices
    ]

    # Test with base=2: localdimensions should be [2^2, 2^1, 2^1, 2^3] = [4, 2, 2, 8]
    grid_custom_base2 = QuanticsGrids.NewDiscretizedGrid(variablenames, indextable;
        lower_bound=(0.0, 0.0), upper_bound=(1.0, 1.0), base=2)
    @test QuanticsGrids.localdimensions(grid_custom_base2) == [4, 2, 2, 8]

    # Test with base=3: localdimensions should be [3^2, 3^1, 3^1, 3^3] = [9, 3, 3, 27]
    grid_custom_base3 = QuanticsGrids.NewDiscretizedGrid(variablenames, indextable;
    ...
    single_site_indextable = [
        [QuanticsGrids.QuanticsIndex(:x, 1), QuanticsGrids.QuanticsIndex(:x, 2), QuanticsGrids.QuanticsIndex(:x, 3)]
    ]
    grid_single_site = QuanticsGrids.NewDiscretizedGrid((:x,), single_site_indextable;
    ...
    complex_indextable = [
        [QuanticsGrids.QuanticsIndex(:x, 1)],                                      # site 1: 1 index -> base^1
        [QuanticsGrids.QuanticsIndex(:y, 1), QuanticsGrids.QuanticsIndex(:z, 1)], # site 2: 2 indices -> base^2
        [QuanticsGrids.QuanticsIndex(:x, 2), QuanticsGrids.QuanticsIndex(:y, 2), QuanticsGrids.QuanticsIndex(:z, 2)], # site 3: 3 indices -> base^3
        [QuanticsGrids.QuanticsIndex(:x, 3), QuanticsGrids.QuanticsIndex(:y, 3)]  # site 4: 2 indices -> base^2
    ]
    grid_complex = QuanticsGrids.NewDiscretizedGrid((:x, :y, :z), complex_indextable;
...
@testitem "quanticsfunction" begin
    ...
    variablenames = (:x, :y)
    indextable = [
        [QuanticsGrids.QuanticsIndex(:x, 1)],
        [QuanticsGrids.QuanticsIndex(:x, 2)],
        [QuanticsGrids.QuanticsIndex(:y, 1)],
        [QuanticsGrids.QuanticsIndex(:y, 2)]
    ]
    grid_custom = QuanticsGrids.NewDiscretizedGrid(variablenames, indextable;
    ...
    complex_indextable = [
        [QuanticsGrids.QuanticsIndex(:x, 1)],
        [QuanticsGrids.QuanticsIndex(:y, 1), QuanticsGrids.QuanticsIndex(:z, 1)],
        [QuanticsGrids.QuanticsIndex(:x, 2), QuanticsGrids.QuanticsIndex(:y, 2)],
        [QuanticsGrids.QuanticsIndex(:z, 2), QuanticsGrids.QuanticsIndex(:x, 3), QuanticsGrids.QuanticsIndex(:y, 3)]
    ]

    # Create grid with base=3, non-unit domain
    grid_complex = QuanticsGrids.NewDiscretizedGrid(complex_variablenames, complex_indextable;
    ...
    coord_mid = QuanticsGrids.quantics_to_origcoord(grid_complex, quantics_mid)
...
@testitem "unfoldingscheme" begin
    ...
    grid_fused = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), unfoldingscheme=:fused)
    grid_interleaved = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), unfoldingscheme=:interleaved)

    # Test 2D grids with different unfolding schemes
    grid_2d_fused = QuanticsGrids.NewDiscretizedGrid((R, R); lower_bound=(0.0, 0.0), upper_bound=(1.0, 1.0), unfoldingscheme=:fused)
    grid_2d_interleaved = QuanticsGrids.NewDiscretizedGrid((R, R); lower_bound=(0.0, 0.0), upper_bound=(1.0, 1.0), unfoldingscheme=:interleaved)
    ...
    grididx = (2, 2)
    quantics_fused = QuanticsGrids.grididx_to_quantics(grid_2d_fused, grididx)
    quantics_interleaved = QuanticsGrids.grididx_to_quantics(grid_2d_interleaved, grididx)
    ...
    coord_fused = QuanticsGrids.quantics_to_origcoord(grid_2d_fused, quantics_fused)
    coord_interleaved = QuanticsGrids.quantics_to_origcoord(grid_2d_interleaved, quantics_interleaved)
    @test coord_fused == coord_interleaved

    # Test default unfoldingscheme (should be :fused)
    grid_default = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,))
...
@testitem "includeendpoint" begin
    ...
    grid_without = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), includeendpoint=false)
    grid_with = QuanticsGrids.NewDiscretizedGrid{1}(R; lower_bound=(0.0,), upper_bound=(1.0,), includeendpoint=true)
    ...
    @test QuanticsGrids.grididx_to_origcoord(grid_with, 2^R) == 1.0
    @test QuanticsGrids.grididx_to_origcoord(grid_without, 2^R) ≈ 1.0 - (1.0 / 2^R)

    # Test 2D case
    grid_2d_without = QuanticsGrids.NewDiscretizedGrid((R, R);
        lower_bound=(0.0, 0.0),
        upper_bound=(2.0, 3.0),
        includeendpoint=false)
    grid_2d_with = QuanticsGrids.NewDiscretizedGrid((R, R);
        lower_bound=(0.0, 0.0),
        upper_bound=(2.0, 3.0),
    ...
    @test QuanticsGrids.grididx_to_origcoord(grid_2d_with, (2^R, 2^R)) == (2.0, 3.0)
    ...
    grid_complex_without = QuanticsGrids.NewDiscretizedGrid(complex_variablenames, complex_indextable;
        lower_bound, upper_bound, base=3, includeendpoint=false)
    grid_complex_with = QuanticsGrids.NewDiscretizedGrid(complex_variablenames, complex_indextable;
    ...
    coord_max_without = QuanticsGrids.quantics_to_origcoord(grid_complex_without, max_quantics_without)
    coord_max_with = QuanticsGrids.quantics_to_origcoord(grid_complex_with, max_quantics_with)
    ...
    @test all(QuanticsGrids.quantics_to_origcoord(grid_complex_without, min_quantics) .≈ lower_bound)
    @test all(QuanticsGrids.quantics_to_origcoord(grid_complex_with, min_quantics) .≈ lower_bound)

    # Test intermediate quantics values behave consistently
    mid_quantics = [2, 5, 14, 5]  # roughly middle values for each site
    coord_mid_without = QuanticsGrids.quantics_to_origcoord(grid_complex_without, mid_quantics)
    coord_mid_with = QuanticsGrids.quantics_to_origcoord(grid_complex_with, mid_quantics)
...
@testitem "boundary error handling" begin
    R = 5
    g = QuanticsGrids.NewDiscretizedGrid{1}(R)
    @test_throws BoundsError QuanticsGrids.origcoord_to_grididx(g, -0.1)
    @test_throws BoundsError QuanticsGrids.origcoord_to_grididx(g, 1.1)
end

@testitem "large grids" begin
    R = 62
    g = QuanticsGrids.NewDiscretizedGrid{1}(R)
    @test QuanticsGrids.grididx_to_quantics(g, 2^R) == fill(2, R)
end
...
@testitem "grid representation conversion _NEW_" begin
    ...
    testset = [
        (QuanticsGrids.NewDiscretizedGrid{1}(R, -3.2, 4.8), 2),
        (QuanticsGrids.NewDiscretizedGrid{2}(R, (-2.3, 1.2), (9.0, 3.5)), (2, 3)),
        (QuanticsGrids.NewDiscretizedGrid(varnames, tupletable), (7, 2)),
    ]
